        ...
        success: function (data) {
            ...
            if(typeof data.status !== "undefined"){
                Object.entries(data.supplier_brands).forEach(entry => {
                    ...
                    brand_data.push('<div class="list-group-item" data-id=key data-label="' + value + '" draggable="true">' + value + '</div>');
                });
                ...
                if(data.supplier_categories != null) {
                    Object.entries(data.supplier_categories).forEach(entry => {
                        const [key, value] = entry;
                        category_data.push('<div class="list-group-item" data-id=key data-label="' + value + '" draggable="true">' + value + '</div>');
                    });
                    var clusterize = new Clusterize({
                        rows: category_data,
                ...
                Object.entries(data.supplier_models).forEach(entry => {
                    const [key, value] = entry;
                    model_data.push('<div class="list-group-item" data-id=key data-label="' + value + '" draggable="true">' + value + '</div>');
                });
                var clusterize = new Clusterize({
                    rows: model_data,
